...
JSAPI.prototype.hasAttrLocal = function (localName, val) {
  ...
  const testName   = ({ name })        => parseName(name).local === localName;
  const testNumStr = ({ name, value }) => parseName(name).local === localName && val ==   value;
  const testRegExp = ({ name, value }) => parseName(name).local === localName && val.test(value);
  const testUserFn = ({ name, value }) => parseName(name).local === localName && val     (value);

  switch (val != null && val.constructor && val.constructor.name) {
    case 'Number': // same as String
    case 'String':
      return this.someAttr(testNumStr);
    case 'RegExp':
      return this.someAttr(testRegExp);
    case 'Function':
      return this.someAttr(testUserFn);
    default:
      return this.someAttr(testName);
  }
};
