 ...
 * @return {string}
 */
function cleanupOutData(data, params, command) {
  var str = '',
    delimiter = '', // no extra space in front of first number
    prev = 0;

  const no_extra_space = params.noSpaceAfterFlags && (command === 'A' || command === 'a');
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (
      params.negativeExtraSpace && delimiter &&
...
exports.encodeSVGDatauri = encodeSVGDatauri;
exports.decodeSVGDatauri = decodeSVGDatauri;
